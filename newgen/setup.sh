...
# Clean any stale dpkg / apt locks and kill hung lock holders
fuser -k -9 /var/lib/dpkg/lock-frontend 2>/dev/null || true
...
# Verify if required tools are already present; if so, skip apt-get to prevent lock conflicts
if command -v ffmpeg >/dev/null 2>&1 && command -v wget >/dev/null 2>&1 && command -v git >/dev/null 2>&1 && python3 -m venv -h >/dev/null 2>&1; then
    echo "All core system dependencies (ffmpeg, wget, unzip, git, python3-venv) are already installed. Proceeding..."
else
    apt-get update -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" || true
    apt-get install -y --fix-missing \
        -o Dpkg::Options::="--force-confdef" \
        -o Dpkg::Options::="--force-confold" \
        ffmpeg wget unzip git python3-pip python3-venv git-lfs || true
fi
